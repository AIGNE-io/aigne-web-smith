...
终端播放器是一个专门用于在网站上显示交互式、预录制的终端会话的组件。该组件是演示命令行指令、展示软件安装过程或以清晰可回放的格式引导用户完成技术流程的有效工具。

此组件使用 `asciinema` 格式创建的录制文件，确保了对终端输出的轻量级、基于文本的捕获。
...
要使用终端播放器，您必须首先创建一个录制文件。推荐的工具是 `asciinema`，一个用于录制和分享终端会话的开源命令行实用程序。
...
*   **终端尺寸：** 播放器将复制用于录制的终端的列和行尺寸。请确保您的终端窗口尺寸适当，以防止在回放期间内容换行或被截断。
...
4.  **添加到项目：** 将下载的 JSON 文件放入您网站的媒体或资源目录中。
 
## 使用录制
 
一旦将 JSON 录制文件放置在项目的媒体或资源目录中，AI 就可以使用它了。无需在 YAML 文件中手动配置组件。
 
只需在您的内容源文件中描述需要终端演示即可。当您运行 `aigne web generate` 或 `aigne web update` 时，AI 将自动找到相关的 `.json` 录制文件，并使用终端播放器组件将其显示在您的网站上。
