...
本指南是您快速搭建可用网站的最佳路径：安装 AIGNE CLI、收集源材料、运行 `aigne web generate`、批准计划并审查生成的页面——所有操作一站式完成。

## 查看端到端流程

<!-- DIAGRAM_IMAGE_START:flowchart:4:3:1765349001 -->
![入门指南](assets/diagram/getting-started-01.zh.jpg)
<!-- DIAGRAM_IMAGE_END -->

## 前提条件

- **Node.js 20 或更新版本**（包含 npm）。使用 `node -v` 检查版本。
...
需要测试版吗？使用 `npm install -g @aigne/cli@beta` 进行安装，并运行 `aigne web upgrade --beta` 以获取最新的 agents。

## 第 2 步：创建项目工作区

为您的第一个网站创建一个干净的目录，并切换到该目录中：
...
将您希望 AI 读取的文档、简报和媒体文件复制或创建到此文件夹（或附近的 `sources` 目录）中。定义明确的材料能显著提高输出质量，因此请花几分钟时间遵循[准备清单](./reference-prepare-materials.md)。
...
4.  **语言** - 选择主要区域设置和任何要生成的译文。
5.  **页面目录** - 生成文件的存储位置（默认为 `aigne/web-smith/pages`）。
6.  **源路径** - 包含您的产品知识的目录或文件。
7.  **自定义规则** - 约束条件，如语气、术语或合规性提醒。

您的回答将被写入 `.aigne/doc-smith/config.yaml`，以便未来的运行可以复用它们。
...
> **重要提示：** `sourcesPath` 控制 AI 可以引用的所有内容。请将其指向包含您的简报、规格、定价表和证明材料的精确目录，以确保生成的文案准确无误。

## 第 5 步：批准计划并检查页面

在读取您的配置后，WebSmith 会提出一个网站地图（页面、章节和关键要点）。批准它，让 AI 起草内容并组装 Astro/React 模板。运行结束后，您将看到：

- 生成的文件位于您选择的 `pagesDir` 目录中。
- 每个页面的日志，显示了使用了哪些源文件。
- 后续操作的建议（发布、翻译或迭代）。

在本地打开生成的页面，或直接继续阅读[发布网站](./guides-publish-website.md)指南以将其上线。
