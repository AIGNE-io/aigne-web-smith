...
本指南將引導您使用單一指令，將您的原始資料轉換為一個可上線的網站。您將檢視可用的 `aigne web generate` 旗標，核准 AI 產生的網站地圖，並了解完成的頁面在磁碟上的存放位置。

如果您需要快速參考所有旗標和別名，請執行 `aigne web -h`；您應該會看到如下的範例輸出。

![`aigne web -h` 的輸出，顯示可用的旗標和別名](../../../assets/images/web-smith-help.png)

## 產生流程

`aigne web generate` 指令會啟動整個網站建立流程。它會讀取您的設定，分析您的原始資料，規劃網站結構，為每個頁面產生內容，並組合最終的檔案。

### 使用方式

若要執行產生流程，請在您的終端機中執行以下指令：

```bash 產生網站 icon=lucide:terminal
aigne web generate
```
...
2.  **分析來源**：AI 會掃描您設定檔中 `sourcesPath` 指定的文件、Markdown 檔案和其他資料，以了解主題內容。
3.  **規劃網站結構**：根據目的、受眾和來源內容，AI 會為您的網站提出一個合乎邏輯的網站地圖，概述所有頁面及其層級結構。在內容產生開始前，系統會提示您檢視並核准此結構。

![在核准產生前檢視建議的網站結構](../../../assets/images/web-smith-generate-structure.png)

4.  **產生頁面內容**：對於已核准結構中的每個頁面，AI 會產生詳細內容，包括標題、描述，以及由專業元件（如主視覺橫幅、功能列表和常見問題）組成的區塊。
5.  **儲存網站檔案**：每個頁面的最終結構化內容會以 YAML 檔案的形式，儲存到您設定檔中 `pagesDir` 指定的目錄中。這些檔案現在已準備好發布。

![產生執行摘要，顯示成功輸出和後續步驟](../../../assets/images/web-smith-generate-success.png)

### 參數

`generate` 指令接受數個可選參數，以自訂其行為。

<x-field-group>
  <x-field data-name="glossary" data-type="string" data-required="false">
    <x-field-desc markdown>指定詞彙表檔案的路徑（例如：`@glossary.md`）。這能確保專案特定術語在產生的內容中一致使用。</x-field-desc>
  </x-field>
  <x-field data-name="forceRegenerate" data-type="boolean" data-required="false">
    <x-field-desc markdown>若設定為 `true`，指令將從頭重新產生所有頁面，覆寫任何先前產生的檔案。這在對 `config.yaml` 檔案或來源文件進行重大變更後很有用。</x-field-desc>
  </x-field>
</x-field-group>

**帶參數的範例：**

```bash # 重新產生網站 icon=lucide:terminal
# 使用詞彙表檔案重新產生整個網站
...
<x-field-group>
  ...
  <x-field data-name="websiteScale" data-type="string" data-required="true">
    <x-field-desc markdown>定義網站期望的規模與複雜度。選項包括 `singlePage`、`minimal`、`standard`、`comprehensive` 和 `aiDecide`。</x-field-desc>
  </x-field>
  ...
  <x-field data-name="sourcesPath" data-type="array" data-required="true">
    <x-field-desc markdown>一個指向您來源內容的檔案路徑或 glob 模式陣列。AI 將分析這些檔案以產生網站。</x-field-desc>
  </x-field>
</x-field-group>

### `config.yaml` 範例

以下是一個完整的 `config.yaml` 檔案範例，並附有註解解釋每個部分。
...
#   minimal         - 最小規模（2-6 頁）：僅包含核心頁面 - 快速上線
#   standard        - 標準規模（7-12 頁）：包含主要區塊的完整網站 [建議]
#   comprehensive   - 綜合規模（12+ 頁）：功能齊全、內容詳細的網站
...
sourcesPath:  # 要分析的原始碼路徑
  - ./docs/**/*.md
  - ./README.md
defaultDatasources:  # 每個頁面預設包含的資料來源
  - ./media.md
...
透過結合 `generate` 指令和一個定義完善的 `config.yaml` 檔案，您可以有效率地產生一個完全符合您規格的完整網站。這個流程自動化了網站結構和內容創作的繁重工作，讓您可以專注於提供高品質的原始資料。

在產生您的網站後，下一步就是將其發布上線。
