...
使用本指南运行 `aigne web translate`，选择您想要的页面和语言，并保存本地化版本，以便与您的主站点一同发布。

AIGNE WebSmith 提供了一种强大而直接的方式，可以自动将您的网站内容翻译成多种语言，让您能以最少的工作量触及全球受众。`translate` 命令使用 AI 为您现有的页面生成高质量的翻译。

本文档将指导您完成内容翻译的全过程，从基本的交互式使用到使用命令行参数的更高级选项。

## 翻译工作原理

翻译过程被设计得既简单又高效。当您运行 `translate` 命令时，WebSmith 会执行以下步骤：
...
4.  **生成翻译**：AI 会处理每个选定页面的内容，并为每种选定的语言生成一个翻译版本。它可以使用一个可选的术语表来确保术语的一致性。
...
```bash 本地化网站 icon=lucide:terminal
aigne web translate
```

![交互式提示，显示已选择多种语言进行翻译](../../../assets/images/web-smith-translate.png)
...
<x-field-group>
  ...
  <x-field data-name="--langs" data-type="array" data-required="false">
    <x-field-desc markdown>一个包含要翻译成的目标语言代码的数组。如果省略，系统将提示您以交互方式选择语言。有关支持的语言完整列表，请参见下表。</x-field-desc>
  </x-field>
  ...
  <x-field data-name="--feedback" data-type="string" data-required="false">
    <x-field-desc markdown>向 AI 提供反馈，以优化和改进现有翻译。这对于进行修正或调整语气非常有用。</x-field-desc>
  </x-field>
</x-field-group>
...
```bash 翻译指定页面 icon=lucide:terminal
...
```bash 使用术语表翻译 icon=lucide:terminal
...
```bash 使用术语表文件翻译 icon=lucide:terminal
...
翻译完您的内容后，下一步自然是将其提供给您的用户。请在[发布网站](./guides-publish-website.md)部分了解如何部署您的多语言站点。