...
本指南將說明如何設定 `aigne web prefs`，包含如何列出已儲存的規則、啟用或停用它們，以及移除不再需要的項目。
...
若要檢視所有目前已儲存的偏好設定，請使用 `list` 指令。此指令會顯示一個包含所有偏好設定規則的表格，包括其 ID、啟用狀態、範圍和規則本身。
...
輸出結果將是一個格式化的表格，讓您輕鬆檢視已儲存的規則。

| ID | Active | Scope | Rule |
| :--------- | :----- | :-------- | :----------------------------------------------------------------- |
| pref_a1b2c3d4 | true | page | 以正式、專業的語氣撰寫。 |
| pref_e5f6g7h8 | true | structure | 不要為過時的「舊版 API 參考」生成頁面或章節。 |
| pref_i9j0k1l2 | false | theme | 醫療保健網站必須使用暖色調。 |

## 切換偏好設定

如果您想暫時停用某個偏好設定而不永久刪除它，可以使用 `toggle` 指令。此指令會切換規則的 `active` 狀態。若要切換特定規則，您必須使用 `--id` 參數提供其唯一的 ID。
...
再次執行 `aigne web prefs list` 將會顯示 `pref_i9j0k1l2` 的狀態為 `false`。再次對同一個 ID 執行 toggle 指令將會重新啟用它。

## 移除偏好設定

若要永久刪除不再需要的偏好設定，請使用 `remove` 指令。此操作不可撤銷。您必須透過 `--id` 參數提供其 ID 來指定要刪除的規則。
...
偏好設定是根據其指定的 `scope`（範圍）來應用的，這決定了規則在何種情境下被觸發。

| Scope | Description |
| :------------ | :------------------------------------------------------------------------------------------------------ |
| **global** | 適用於生成和內容優化的所有階段。用於通用的風格或內容規則。 |
| **structure** | 僅在規劃或更新網站結構（例如新增、移除頁面）時適用。 |
| **page** | 在生成或優化個別頁面內容時適用。 |
| **translation** | 僅在內容翻譯過程中適用。 |
| **theme** | 在生成或修改網站的視覺主題（如顏色和字體）時適用。 |

## 總結

`prefs` 指令是客製化和控制 AIGNE WebSmith 長期行為的重要工具。透過列出、切換和移除偏好設定，您可以維護一組清晰有效的規則，確保 AI 持續產出符合您專案特定需求的結果。