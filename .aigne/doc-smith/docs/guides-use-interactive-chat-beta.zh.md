...
> **注意：** 交互式聊天功能目前处于测试阶段。虽然功能齐全，但您可能会遇到意外行为。在我们不断改进此体验的过程中，感谢您的反馈。
...
交互式聊天会话由一个专用的 AI agent 提供支持，该 agent 能够理解您的请求并使用其他 WebSmith 工具来执行任务。它会保留您的对话记忆，让您可以进行顺序更改并在先前的操作基础上继续构建。
...
交互式聊天是管理整个网站生命周期的强大方式，无需记住特定的命令和参数。如果需要更直接的控制，您可以随时使用后续章节中详细介绍的标准命令。
