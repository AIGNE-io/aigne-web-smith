...
<x-cards>
  ...
  <x-card data-title="素材の準備" data-icon="lucide:folder-check" data-href="./reference-prepare-materials">
    ソースファイルの整理と素材準備のベストプラクティス。高品質な生成のための要件、ドキュメント、アセットの収集方法を学びます。
  </x-card>
  <x-card data-title="ウェブサイトの作成" data-icon="lucide:rocket" data-href="./guides-create-website">
    ウェブサイトを作成するためのステップバイステップガイド。効果的なルールの記述と適切な設定の選択に関する詳細な例が含まれています。
  </x-card>
</x-cards>
