...
export default async function chooseContents(input = {}, options = {}) {
  ...
  const targetsDefinition = {
    ...
    deploymentConfig: {
      ...
      description: ({ finalConfigPath }) => `Delete appUrl from './${toDisplayPath(finalConfigPath)}'.`,
  ...
  const normalizedTargets = Array.isArray(rawTargets) ? rawTargets.filter((target) => !!targetsDefinition[target]) : []
  ...
  const message = [header, '\n', detailLines, suggestions.length ? '' : null, suggestions.join('\n')]
    .filter(Boolean)
    .join('\n')
