...
import { DEFAULT_EXCLUDE_PATTERNS } from '../../utils/constants.mjs'
...
export default async function loadConfig({ config, appUrl }) {
  ...
  try {
    ...
    parsedConfig.projectSlug = slugify(transliterate(parsedConfig?.projectSlug || parsedConfig.projectName), {
      lower: true,
      strict: true,
    })

    // collect locales
    const locales = Array.from(new Set([parsedConfig?.locale, ...(parsedConfig?.translateLanguages || [])])).filter(
      Boolean,
    )
